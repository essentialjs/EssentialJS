...
ko.utils = new (function () {
    ...
    return {
        ...
        evalWithinScope: function (expression, scope) {
            ...
            return (new Function("sc", ["with(sc) { return (", expression, ") }"].join("")))(scope);
        },
        ...
        registerEventHandler: function (element, eventType, handler, bClean) {
...
ko.utils.domNodeDisposal = new (function () {
    ...
    function cleanSingleNode(node) {
        ...
        if(node.handler)
        {
        	for(var i = 0; i < node.handler.length; i++)
    		{
        		node.handler[i](true);
    		}
        	node.handler = null;
...
ko.isWriteableObservable = function (instance) {
    ...
    // Observable
    if ((instance.__ko_proto__ === ko.observable) && (instance.setUserEnteredValue))
        return true;
...
ko.jsonExpressionRewriting = (function () {
    ...
    return {
        parseJson: function (jsonString) {
// TODO: we need to understand what to do with this method -- short-cutting it leads to large
// performance gains for some use cases without any observed impact in functionality!
return null;
            jsonString = ko.utils.stringTrim(jsonString);
        ...
        insertPropertyAccessorsIntoJson: function (jsonString) {
            ...
            if (propertyAccessorTokens.length > 0) {
                pJsonString = pJsonString.concat(", '_ko_property_writers' : { ", propertyAccessorTokens, " } ");
            }
            pJsonString.push(" } ");
            
            return pJsonString.join("");
        }
    };
})();
...
ko.bindingHandlers['value'] = {
    'init': function (element, valueAccessor, allBindingsAccessor) { 
        ...
        ko.utils.arrayForEach(eventsToCatch, function(eventName) {
            ...
            ko.utils.registerEventHandler(element, eventName, function (bClean) {
                runEventHandler(function() {
                	...
                	else
            		{
	                    ...
	                    if (ko.isWriteableObservable(modelValue))
	                    	modelValue.setUserEnteredValue(elementValue);
	                    else {
	                        var allBindings = allBindingsAccessor();
    ...
    'init': function (element, valueAccessor, allBindingsAccessor) {
        ko.utils.registerEventHandler(element, "change", function () {
            ...
            if (ko.isWriteableObservable(value)){ 
            	var wibble = ko.bindingHandlers['selectedOptions'].getSelectedValuesFromSelectNode(this);
            	value.setUserEnteredValue(wibble);
            }
            else {
                var allBindings = allBindingsAccessor();
                if (allBindings['_ko_property_writers'] && allBindings['_ko_property_writers']['value'])
        ...
        if (newValue && typeof newValue.length == "number") {
            ...
            for (var i = 0, j = nodes.length; i < j; i++) {
                ...
                if (node.tagName == "OPTION")
                    ...
                    ko.utils.setOptionNodeSelectionState(node, ko.utils.arrayIndexOf(newValue, nodeValue) >= 0);
            }
        }
    }
};
...
ko.bindingHandlers['checked'] = {
    'init': function (element, valueAccessor, allBindingsAccessor) {
        var updateHandler = function() {            
            ...
            if ((element.type == "checkbox") && (ko.utils.unwrapObservable(modelValue) instanceof Array)) {
                ...
                if (ko.utils.unwrapObservable(modelValue) !== valueToWrite) { // Suppress repeated events when there's nothing new to notify (some browsers raise them)
                	modelValue.setUserEnteredValue(valueToWrite);
                }
            } else {
                var allBindings = allBindingsAccessor();
    ...
    'update': function (element, valueAccessor) {
        ...
        if (element.type == "checkbox") {        	
            ...
            if (value && ko.utils.isIe6 && element.checked) 
        ...
        for (var attrName in value) {
            if (typeof attrName == "string") {
                ...
                else
            	{
                	if (attrName == "class")
                	{
                		element.className = attrValue.toString();
                	}
                	else
                	{
...
ko.templateRewriting = (function () {
    var memoizeBindingAttributeSyntaxRegex = /(<[a-zA-Z]+\d*(\s+(?!data-bind=)[a-z0-9\-]+(=(\"[^\"]*\"|\'[^\']*\'|[^>]*))?)*\s+)data-bind=(["'])([\s\S]*?)\5/gi;
...
ko.jqueryTmplTemplateEngine = function () {
    ...
    this['getTemplateNode'] = function (template) {
        var templateNode = caplin.services.HTMLResourceService.getHTMLTemplate(template);
        if (!templateNode)
    ...
    this['rewriteTemplate'] = function (template, rewriterCallback) {
        ...
        var rewritten = rewriterCallback(templateNode.text || templateNode.innerHTML);     
